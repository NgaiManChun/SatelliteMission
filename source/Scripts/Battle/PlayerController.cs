...
// PlayerController
// -------------------------------------------------------
// プレイヤー入力とカメラ制御を管理するクラス
//
// 移動、攻撃、ジャンプ、ステップ、飛行、ロックオン補助、
// カメラ操作などを処理する
// =======================================================
    ...
    void Update()
    {
        ...
        if (BattleSequenceController.GetSceneState() == SCENE_STATE.PLAYING)
        {
            ...
            if (hasGamepad)
            {
                ...
                main_attack =
                    main_attack || Gamepad.current.rightShoulder.isPressed;

                sub_attack =
                    sub_attack || Gamepad.current.rightTrigger.isPressed;

                jump_down =
                    jump_down || Gamepad.current.leftTrigger.wasPressedThisFrame;

                fly =
                    fly || Gamepad.current.leftTrigger.isPressed;

                lockon_assist_onoff =
                    lockon_assist_onoff || Gamepad.current.yButton.wasPressedThisFrame;

                pause =
                    pause || Gamepad.current.startButton.wasPressedThisFrame;
            }
            ...
            double_input_count =
                Mathf.Min(
                    double_input_count + Time.deltaTime,
                    double_input_time + 1.0f
                );

            if (axis_hit)
            {
                if (raw_move == double_input_direction &&
                   double_input_count <= double_input_time)
                {
        ...
        if (!BattleSequenceController.isPause)
        {
            ...
            {
                camera_mode =
                    (camera_mode == CAMERA_MODE.FREE_ANGLE)
                    ? CAMERA_MODE.LOCK_TARGET
                    : CAMERA_MODE.FREE_ANGLE;
            }
            ...
            CharacterCameraController characterCameraController =
                (player_character)
                ? player_character.GetComponent<CharacterCameraController>()
                : null;

            Camera camera =
                (characterCameraController)
                ? characterCameraController.camera
                : null;
            ...
            if (camera)
            {
                ...
                CharacterModel[] enemies =
                    BattleSequenceController.GetEnemies(player_character.team);
                ...
                if (reset_look_at)
                {
                    Vector3 player_position =
                        player_character.gameObject.GetCenterPoint();

                    camera_offset =
                        player_character.transform.rotation * camera_offset;
                }
                ...
                {
                    Vector3 pointer_move =
                        new Vector3(
                            -pointer_y * Options.camera_speed,
                            pointer_x * Options.camera_speed,
                            0
                        );

                    pointer_move =
                        camera.transform.TransformDirection(pointer_move);

                    Vector3 player_position =
                        player_character.gameObject.GetCenterPoint();

                    camera_offset =
                        Quaternion.Euler(pointer_move) * camera_offset;
                    ...
                    camera.transform.rotation =
                        Quaternion.LookRotation(
                            player_position - camera.transform.position
                        ) * Quaternion.Euler(-15, 0, 0);

                    look_at =
                        camera.transform.position +
                        camera.transform.forward * free_angle_focus_distance;
                    ...
                    foreach (CharacterModel enemy in enemies)
                    {
                        Vector3 enemy_position =
                            enemy.gameObject.GetCenterPoint();

                        Vector3 screenPos =
                            camera.WorldToScreenPoint(enemy_position);

                        screenPos.x =
                            (screenPos.x - camera.scaledPixelWidth / 2) /
                            characterCameraController.canvas.scaleFactor;
                        ...
                        if (screenPos.z > 0 &&
                            Mathf.Abs(screenPos.x) < lockon_assist_area.x / 2 &&
                            Mathf.Abs(screenPos.y) < lockon_assist_area.y / 2)
                        {
                ...
                else if (camera_mode == CAMERA_MODE.LOCK_TARGET)
                {
                    if (enemies.Length > 0)
                    {
                        ...
                        Vector3 target_position =
                            enemies[0].gameObject.GetCenterPoint();
                ...
                Vector3 move_direction =
                    camera.transform.TransformDirection(
                        new Vector3(move_x, 0, move_y)
                    );
        ...
        if (pause)
        {
            BattleSequenceController.isPause =
                !BattleSequenceController.isPause;
        }
    }
