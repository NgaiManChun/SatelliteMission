...
public class RadarDish : CharacterModel
{
    ...
    protected override void BeforeUpdate()
    {
        ...
        if (moving_attack_main)
        {
            ...
            {
                GameObject cube_bot_obj = (GameObject)Resources.Load("cube_bot");
                CharacterModel model = (CharacterModel)cube_bot_obj.GetComponent<CharacterModel>();
                CubeBotController cube_bot_controller = (CubeBotController)cube_bot_obj.GetComponent<CubeBotController>();
                cube_bot_controller.pattren_type = CUBE_BOT_PATTREN_TYPE.DEFENSE_AREA;
                cube_bot_controller.defense_position = transform.position;

                model.team = this.team;
                Instantiate(cube_bot_obj, gameObject.GetCenterPoint() + transform.forward * 4.0f, Quaternion.identity);

                cube_bot_current_load = Mathf.Max(cube_bot_current_load - 1, 0.0f);
        ...
        if (moving_attack_sub)
        {
            ...
            {
                hp = Mathf.Min(hp + hp_recover_amount, max_hp);
                hp_recover_current_load = Mathf.Max(hp_recover_current_load - 1, 0.0f);
    ...
    protected override void AfterUpdate()
    {
        ...
        cube_bot_current_load = Mathf.Min(cube_bot_current_load + cube_bot_recover_rate * Time.fixedDeltaTime, cube_bot_max_load);
        cube_bot_next_fire_processed = Mathf.Min(cube_bot_next_fire_processed + cube_bot_fire_rate * Time.fixedDeltaTime, 1.0f);

        hp_recover_current_load = Mathf.Min(hp_recover_current_load + hp_recover_recover_rate * Time.fixedDeltaTime, hp_recover_max_load);
        hp_recover_next_fire_processed = Mathf.Min(hp_recover_next_fire_processed + hp_recover_fire_rate * Time.fixedDeltaTime, 1.0f);
    }
